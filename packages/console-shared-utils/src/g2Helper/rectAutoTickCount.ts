 ...
 * */
function rectAutoTickCount(chart, config, defs, transpose) {
  // X轴&Y轴对应的长度
  const xLen = transpose ? chart.get('height') : chart.get('width');
...
function computerAxis(chart, axisConfig, def, isVertical, len) {
  ...
  if (
    !axisConfig ||
    (axisConfig && axisConfig.visible === false) ||
    axisConfig.tickCount !== 'auto'
  ) {
    return;
  }

  let labelFontSize = pxToNumber('12px');
  if (
    axisConfig.customConfig &&
    axisConfig.customConfig.label &&
    axisConfig.customConfig.label.textStyle &&
    axisConfig.customConfig.label.textStyle.fontSize
  ) {
  ...
  if (isVertical) {
    ...
    const labelWidth = ((def.mask || 'YYYY-MM-DD HH:mm:ss').length * labelFontSize) / 2;
...
function convertFontSize(fontSize) {
  ...
  if (isInvalidNumber(size)) {
    return pxToNumber('12px');
  } else {
    return Number(size);
  }
}

export { rectAutoTickCount };
